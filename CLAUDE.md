...
**Faz 5 sürüyor** — Frontend. Mimari kararlar netleşti (bkz. madde 11 aşağıda): tek Next.js projesi (App Router), `/admin` route bazlı ayrım, shadcn/ui (bu sürümde Radix değil Base UI tabanlı), WMS'ten taşınan zustand+axios auth deseni, herkese açık sayfalarda SSR (SEO için) + kişiselleştirilmiş sayfalarda Client Component. Proje iskeleti, **auth sayfaları** (login/register), **public katalog** (ana sayfa/kategori/ürün detay, üçü de SSR), **sepet** (guest+authenticated akış, header badge'i, ürün detayına "Sepete Ekle") ve **adres defteri** (`account/addresses` CRUD) tamamlandı, tarayıcıda uçtan uca doğrulandı. Son iki checkpoint (sepet, adres defteri) backend'de hiç değişiklik gerektirmedi — Cart ve Identity/Address modülleri Faz 2'den beri ihtiyaç duyulan her şeyi zaten sunuyordu. Ondan önceki iki checkpoint sırasında eksik çıkan iki parça tamamlanmıştı: CORS middleware (monolith + payment-service) ve "tüm ürünler" listesini dönen bir sorgu (Catalog'un `GetProductsQuery`'si artık `categoryId`'yi opsiyonel alıyor). Kalan checkpoint'ler (checkout, siparişlerim, admin panel) sırayla ilerliyor — detay ve güncel durum için [TASKS.md](./TASKS.md) Faz 5.
