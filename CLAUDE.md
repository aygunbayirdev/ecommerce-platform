...
**Faz 5 sürüyor** — Frontend. Mimari kararlar netleşti (bkz. madde 11 aşağıda): tek Next.js projesi (App Router), `/admin` route bazlı ayrım, shadcn/ui (bu sürümde Radix değil Base UI tabanlı), WMS'ten taşınan zustand+axios auth deseni, herkese açık sayfalarda SSR (SEO için) + kişiselleştirilmiş sayfalarda Client Component. Proje iskeleti, **auth sayfaları** (login/register) ve **public katalog** (ana sayfa/kategori/ürün detay, üçü de SSR) tamamlandı, tarayıcıda uçtan uca doğrulandı. Bu iki checkpoint sırasında backend'de eksik çıkan iki parça da tamamlandı: CORS middleware (monolith + payment-service) ve "tüm ürünler" listesini dönen bir sorgu (Catalog'un `GetProductsQuery`'si artık `categoryId`'yi opsiyonel alıyor). Kalan checkpoint'ler (sepet, checkout, siparişlerim, admin panel) sırayla ilerliyor — detay ve güncel durum için [TASKS.md](./TASKS.md) Faz 5.
