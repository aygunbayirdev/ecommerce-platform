...
**Faz 5 sürüyor** — Frontend. Mimari kararlar netleşti (bkz. madde 11 aşağıda): tek Next.js projesi (App Router), `/admin` route bazlı ayrım, shadcn/ui (bu sürümde Radix değil Base UI tabanlı), WMS'ten taşınan zustand+axios auth deseni, herkese açık sayfalarda SSR (SEO için) + kişiselleştirilmiş sayfalarda Client Component. Proje iskeleti, **auth sayfaları** (login/register), **public katalog** (ana sayfa/kategori/ürün detay, üçü de SSR) ve **sepet** (guest+authenticated akış, header badge'i, ürün detayına "Sepete Ekle") tamamlandı, tarayıcıda uçtan uca doğrulandı. Sepet checkpoint'i backend'de hiç değişiklik gerektirmedi (Cart modülü Faz 2'den beri hem guest hem authenticated akışı destekliyordu); ondan önceki iki checkpoint sırasında eksik çıkan iki parça tamamlanmıştı: CORS middleware (monolith + payment-service) ve "tüm ürünler" listesini dönen bir sorgu (Catalog'un `GetProductsQuery`'si artık `categoryId`'yi opsiyonel alıyor). Kalan checkpoint'ler (adres defteri, checkout, siparişlerim, admin panel) sırayla ilerliyor — detay ve güncel durum için [TASKS.md](./TASKS.md) Faz 5.
