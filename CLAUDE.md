...
**Faz 5 tamamlandı** — Frontend. Mimari kararlar netleşti (bkz. madde 11 aşağıda): tek Next.js projesi (App Router), `/admin` route bazlı ayrım, shadcn/ui (bu sürümde Radix değil Base UI tabanlı), WMS'ten taşınan zustand+axios auth deseni, herkese açık sayfalarda SSR (SEO için) + kişiselleştirilmiş sayfalarda Client Component. Proje iskeleti, **auth sayfaları** (login/register), **public katalog** (ana sayfa/kategori/ürün detay, üçü de SSR), **sepet** (guest+authenticated akış, header badge'i, ürün detayına "Sepete Ekle"), **adres defteri** (`account/addresses` CRUD), **checkout + ödeme** (adres seçimi, kupon kodu, sipariş oluşturma, payment-service'e doğrudan ödeme, onay ekranı), **siparişlerim + takip** (liste, detay/durum geçmişi, kargo takibi, iptal) ve **ürün yorumu** (teslim alınan siparişlerden yorum ekleme) tamamlandı, tarayıcıda uçtan uca doğrulandı. Ürün yorumu checkpoint'i, önceki dördünden (sepet/adres defteri/checkout/siparişlerim) farklı olarak **küçük ama gerçek bir backend değişikliği** gerektirdi: sipariş kalemlerinde (`OrderItemDto`) `ProductId` hiç yoktu (sadece `ProductVariantId`), ve bu bilgi hiçbir public endpoint'ten alınamıyordu — Cart'ın `GetCartByIdQueryHandler`'ının zaten yaptığı cross-module read composition (`ISender` ile Catalog'un `GetProductVariantSummariesQuery`'sine gidip variant'ı product'a bağlamak) `GetOrderByIdQueryHandler`'a da genişletildi (yeni bir mimari desen değil, var olanın ikinci uygulanışı). Ondan önceki checkpoint'te (siparişlerim) bulunan bir bug: `useAddresses`/`useCart`'ın zaten uyguladığı "önce `isAuthenticated` kontrol et, sonra sorguyu `enabled` ile kilitle" deseni `useMyOrders`/`useOrder`/`useShipment`'ta eksikti — düzeltildi. Ondan önceki checkpoint'te (checkout) de iki gerçek bulgu çıkmıştı: seed edilen katalog ürünlerinde hiç stok yoktu (düzeltildi, test fixture) ve `usePayment`'ın polling'i tarayıcı sekmesi arka plandayken react-query'nin varsayılanı yüzünden duruyordu (`refetchIntervalInBackground: true` ile düzeltildi — gerçek bir UX açığıydı, sadece test ortamı garipliği değil). Ondan önceki iki checkpoint sırasında eksik çıkan iki parça tamamlanmıştı: CORS middleware (monolith + payment-service) ve "tüm ürünler" listesini dönen bir sorgu (Catalog'un `GetProductsQuery`'si artık `categoryId`'yi opsiyonel alıyor). **Admin panel — katalog yönetimi** de tamamlandı: `/admin` route'unun ilki (guard + nav), kategori/marka/ürün özelliği CRUD'u, ürün+varyant Deactivate/Reactivate, ürün görseli kaldırma, stok toplu listeleme — backend'in o ana kadar neredeyse "sadece oluştur" olan katalog tarafına ilk gerçek mutasyon/silme/pasife-alma uçları eklendi (detaylı gerekçe ve kapsam kararı için TASKS.md). Bu checkpoint sırasında gerçek bir mimari bulgu çıktı: Inventory'nin stok listesini Catalog'la zenginleştirme denemesi `Catalog.Application ↔ Inventory.Application` dairesel bağımlılığına çarptı (Catalog zaten Faz 2'den Inventory'e referans veriyordu) — çözüm composition'ı Api katmanına (composition root) taşımaktı, Inventory.Application hiç Catalog bilmiyor. Ayrıca bilinçli bir tasarım kararı: `ProductVariantSummaryDto.IsActive` artık sadece varyantın değil ürünün de aktifliğini yansıtıyor (`pv.is_active AND p.is_active`) — bu satır olmadan "Ürünü Pasife Al" Cart'ı hiç etkilemezdi, doğrudan linkle hâlâ sepete eklenebilirdi. Ardından `frontend/` de Docker Compose'a eklendi (ad-hoc, Faz 6'nın küçük bir ön adımı) — `output: 'standalone'`, host portu 3000 değil **3001** (WMS'in kendi frontend container'ı 3000'i kullanıyor, Postgres'in 5433 gerekçesinin aynısı); SSR fetch'inin (`server-fetch.ts`) container içinden `localhost:8080`'e değil Docker servis adı `backend:8080`'e gitmesi gerektiği için `NEXT_PUBLIC_API_URL`'den ayrı, client bundle'ına hiç gömülmeyen bir `INTERNAL_API_URL` eklendi; `next build`'in parametre almayan tek SSR sayfasını (`/`) backend ayakta değilken build-time prerender etmeye çalışıp patladığı bulundu, üç public SSR sayfasına da `export const dynamic = 'force-dynamic'` eklenerek çözüldü. **Admin panel — sipariş/kargo/kupon yönetimi** de tamamlandı: dört modülü (Order/Shipping/Promotion/Review) aynı anda kapsayan en büyük admin checkpoint'i — Order'a admin "tüm siparişleri listele" sorgusu (hiç yoktu) + admin zorla iptal komutu, Coupon'a Reactivate, Review'a reddetme (silme) + moderasyon kuyruğunun ürün adı/yorumcu adıyla zenginleştirilmesi (composition-in-controller deseninin üçüncü tekrarı) eklendi; Shipping'e hiç dokunulmadı (zaten tamdı). Detaylı gerekçe, üç kullanıcı kapsam kararı ve tüm doğrulama adımları için TASKS.md. **Cilalama ve son doğrulama** ile Faz 5 kapandı: `not-found.tsx`/`error.tsx` eklendi; ayrıca eklenen bir `loading.tsx`, Next.js 16.3.0 + Turbopack'in `force-dynamic` sayfaları Suspense'e sardığında streaming'in hiç çözülmeyip **tüm SSR sayfalarının** (ana sayfa dahil) kalıcı yükleniyor durumunda kalmasına yol açtığı bulunup geri alındı — lokal `npm run dev`'de izole edilip doğrulandı, framework seviyesinde bir regresyon, kod hatası değil. Kullanıcının "sitemiz responsive olsun, mobilde de iyi görünsün" isteğiyle iki sistemik bulgu düzeltildi: `/admin` layout'u hiç responsive değildi (sabit `w-48` sidebar → `md:` breakpoint'li dikey/yatay geçiş) ve neredeyse her liste satırı bileşeni (~8 dosya) `flex-wrap` içermiyordu, dar ekranda taşma riski taşıyordu — hepsine eklendi, `CartItemRow` ayrıca iki gruba bölünüp dar ekranda sarılacak şekilde yeniden düzenlendi. Kapanışta yeni bir müşteri hesabıyla kayıttan (kupon + admin sipariş/kargo işleme + yorum onayı dahil) public sayfada yorumun görünmesine kadar tek kesintisiz bir golden path doğrulandı, hiçbir entegrasyon regresyonu bulunmadı.

**Faz 6 sürüyor** — Üretime Hazırlık. **CI/CD** tamamlandı: `.github/workflows/ci.yml`, `pull_request`/`push` (main) tetikleyicili üç paralel job (`backend`, `payment-service`, `frontend` — sırasıyla build+test, build+test, `tsc --noEmit`→lint→test→build). CI'a özgü gerçek bir bulgu çıktı: `LayoutProps` gibi Next.js'in route-typing global tipleri sadece `next dev`/`next build`'in yan etkisi olarak üretiliyor — tamamen temiz bir CI checkout'ta `.next/` hiç olmadığı için `tsc --noEmit` ilk adım olarak patladı, `npx next typegen` adımı `tsc`'den önce eklenerek çözüldü. **Test kapsamını genişletme** de tamamlandı: iki araştırma turu backend genelinde ~20 test edilmemiş handler dalı (çoğunlukla NotFound/Conflict) buldu, en yüksek etkili 5'i kapatıldı (Inventory'nin üç toplu-stok handler'ı, Cart'ın `GetCartByIdQueryHandler`'ı, Order'ın dört `Mark*CommandHandler`'ı, Identity'nin `RefreshTokenCommandHandler`'ı, Promotion'ın `DeactivateCouponCommandHandler`'ı + domain testi) — toplam backend test sayısı 174'e çıktı. Ayrıca **Testcontainers.PostgreSql** ile ilk entegrasyon testleri eklendi (Order + Payment, EF Core yazma/Dapper okuma bütünlüğünü gerçek Postgres'e karşı doğruluyor, RabbitMQ katmanı bilinçli olarak bu checkpoint'in dışında bırakıldı) — detaylı gerekçe, kapsam kararları ve bulunan bir test-izolasyon hatası (paylaşılan container'da çakışan idempotency key) için TASKS.md Faz 6. Sırada: **Deploy** (Hetzner) — detay için [TASKS.md](./TASKS.md).
