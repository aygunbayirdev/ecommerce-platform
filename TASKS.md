...
- [ ] **Identity: Adres Defteri** — `Address` entity zaten var (tablo migration'da mevcut), sadece CQRS eksik: `AddAddressCommand`, `UpdateAddressCommand`, `DeleteAddressCommand`, `SetDefaultAddressCommand`, `GetAddressesByUserIdQuery`. Order modülü checkout'ta bu adresleri kullanacak (adres seçilip **snapshot** olarak Order'a kopyalanır — bkz. CLAUDE.md "Order snapshot mantığı").
- [ ] **Catalog modülü** — Tablolar: `Categories` (self-referencing, hiyerarşik), `Brands`, `Products`, `ProductVariants` (SKU seviyesi, fiyat burada), `ProductImages`, ve dinamik varyant özellik modeli: `ProductAttributes`, `CategoryAttributes` (kategori bazlı hangi attribute geçerli), `ProductVariantAttributeValues`. Gerekçe: sabit Color/Size kolonu yerine kategoriye göre değişen özellik seti (Giyim: Renk/Beden, Elektronik: RAM/Depolama) + faceted filtering desteği.
