...
- [ ] **Catalog modülü — Product/Variant/Image** — Tablolar: `Products`, `ProductVariants` (SKU seviyesi, fiyat burada), `ProductImages`, `ProductVariantAttributeValues` (Category'ye atanmış `ProductAttribute`'ları kullanarak varyant başına dinamik özellik değeri — ör. Renk: Kırmızı, Beden: M). Referans verilere (Category/Brand/ProductAttribute) bağımlı olduğu için ayrı görev olarak bırakıldı. `AddProductVariantCommand` handler'ı, girilen her `ProductAttributeId`'nin ürünün kategorisine atanmış olduğunu doğrulamalı (cross-aggregate kontrol, Application katmanında).
