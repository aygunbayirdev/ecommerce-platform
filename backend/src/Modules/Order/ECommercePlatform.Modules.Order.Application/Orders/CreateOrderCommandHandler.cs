...
/// <summary>
/// Orchestrates checkout across Cart, Identity, and Inventory via synchronous ISender calls — a
/// user-facing action needs an immediate success/failure answer, unlike Catalog → Inventory's
/// eventual-consistency domain event.
///
/// Cross-module atomicity is handled by sequencing, not a distributed transaction: the Order is
/// created and saved FIRST (the commit point), then stock is reserved. If reservation fails, the
/// already-persisted order is transitioned to Cancelled instead of being rolled back — so a
/// failure always leaves an auditable record (a cancelled order) rather than an invisible
/// resource leak (stock reserved with no order to release it).
/// </summary>
