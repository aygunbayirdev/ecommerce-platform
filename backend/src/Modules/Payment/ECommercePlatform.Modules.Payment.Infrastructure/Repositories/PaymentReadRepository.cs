...
{
    private sealed record PaymentRow(Guid Id, Guid OrderId, decimal Amount, PaymentStatus Status);

    public async Task<PaymentDto?> GetByOrderIdAsync(Guid orderId, CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT id AS "Id", order_id AS "OrderId", amount AS "Amount", status AS "Status"
        ...
        return new PaymentDto(payment.Id, payment.OrderId, payment.Amount, payment.Status.ToString(), transactions);
    }
}
