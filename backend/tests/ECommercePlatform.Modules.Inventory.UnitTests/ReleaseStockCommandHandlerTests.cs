...
public sealed class ReleaseStockCommandHandlerTests
{
    ...
    {
        _handler = new ReleaseStockCommandHandler(_stockItemWriteRepository.Object);
    }
    ...
    [Fact]
    public async Task Handle_ShouldReleaseAllItems_OnHappyPath()
    {
        ...
        Assert.Equal(3, stockItemA.AvailableQuantity);
        Assert.Equal(0, stockItemB.ReservedQuantity);
        Assert.Equal(2, stockItemB.AvailableQuantity);
    ...
    [Fact]
    public async Task Handle_ShouldReturnConflictWithoutSaving_WhenReleasingMoreThanReserved()
    {
        var stockItemA = CreateStockItemWithReserved(3);
